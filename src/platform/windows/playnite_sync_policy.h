...
namespace platf::playnite::sync {
  ...
  // hidden_lower holds ids of games flagged hidden in Playnite; known_ids_lower holds every id in
  // the Playnite snapshot. Auto apps whose id is absent from known_ids_lower were deleted from the
  // library and are purged, but only when library_complete says the snapshot is the whole library
  // rather than a partially accumulated one.

  ...
  std::optional<proc::proc_t> parse(const std::string &file_name) {
    ...
    do {
      ...
      try {
        ...
        // Metadata written by a provider (Playnite sync, IGDB fetch). Legacy `playnite-*` keys
        // are still understood; the background path may carry environment variables.
        ctx.metadata = game_metadata::read_app_metadata(app_node);
        ctx.metadata.background_image_path = parse_env_val(this_env, ctx.metadata.background_image_path);

  ...
  std::optional<proc::proc_t> parse(const std::string &file_name) {
    ...
    do {
      ...
      try {
        ...
        // Descriptive metadata passthrough, written by whichever provider owns this app.
        // The container is shared, so this reads the same whether Playnite, the IGDB resolver
        // or the user filled it in.
        ctx.metadata = metadata::read_from_app(app_node);
        ctx.metadata.background_image_path =
          parse_env_val(this_env, ctx.metadata.background_image_path);
